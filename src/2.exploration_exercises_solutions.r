...
set.seed(123)
diamonds_unusual <- diamonds |>
  filter(x < 3 | !between(y, 3, 20) | !between(z, 1, 10))
diamonds2 <- diamonds |>
  slice_sample(n = 2000) |>   #random sample
  bind_rows(diamonds_unusual) #unusual values
ggplot(diamonds2, aes(x = x, y = y)) +
  geom_point(alpha = 0.2)
ggplot(diamonds2, aes(x = x, y = z)) +
  geom_point(alpha = 0.2)
ggplot(diamonds2, aes(x = y, y = z)) +
  geom_point(alpha = 0.2)

rm(diamonds_unusual, diamonds2)
...
diamonds |>
  mutate(ending = round(carat*100) %% 10) |>      #calculate ending algorism of "carat"
  ggplot(aes(x = ending)) +
  geom_histogram(binwidth = 1) + 
  scale_x_continuous(breaks = 0:9)
...
#zooming in x-axis using coord_cartesian()
ggplot(diamonds, aes(x = price)) +
  geom_histogram(binwidth = 500) +
...
ggplot(diamonds, aes(x = price)) +
  geom_histogram(binwidth = 500) +
  xlim(100, 5000) #remove bars outside x-axis interval
...
ggplot(diamonds, aes(x = price)) +
  geom_histogram(binwidth = 500) +
  xlim(100, 5000) +                #remove bars outside x-axis interval
  ylim(0, 3000)                    #remove bars outside y-axis interval
...
diamonds |>
  mutate(
    ...
    y_class = cut(y, breaks = c(3, 5, 7, 9, 11))
  ) |>
  ggplot(aes(x = y_class)) +
  geom_bar()
...
flights |> 
  ...
  mutate(absent = tailnum %in% miss_tailnum) |>
...
diamonds |>
  filter(carat < 3) |>
  ggplot(aes(x = carat, y = price, group = cut_width(carat, 0.1))) +
  geom_boxplot()
...
#geom_freqpoly() with cut_interval()
ggplot(diamonds, aes(color = cut_interval(carat, 3), x = price)) +
  geom_freqpoly(binwidth = 500) +
  labs(x = "Price", y = "Count", color = "Carat")


# Conclusions 1:
# 1. cut_number() makes n groups with equal number of observations
# 2. cut_width() makes groups of specified width
...
diamonds |>
  mutate(carat = cut(carat, breaks = c(0, 1, max(carat)))) |>
